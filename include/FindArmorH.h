...
void FindArmorH(Mat src, vector<Point> &armors)
{
    if(src.channels()!=3)
        return;
...
//    cout<<lights.size()<<endl;
    if (lights.size()>1)
    {
        double sds(lights.size()*lights.size());
